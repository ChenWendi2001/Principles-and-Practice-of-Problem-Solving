...
class kdTree{
    ...
    kdNode* minNode(kdNode* a,kdNode* b,int dimCmp);
    kdNode* maxNode(kdNode* a,kdNode* b,int dimCmp);
public:
    kdNode* root = nullptr;
    kdTree();
    ~kdTree();
...
class Trie {
    char END;
    void insert(string word,TrieNode* t,int n);
    bool search(string word,TrieNode* t,int n);
public:
    TrieNode* root;
    Trie();
    ~Trie();
